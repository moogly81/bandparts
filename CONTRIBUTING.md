...
Never commit sheet music. Everything under `data/` is git-ignored because it
holds copyrighted scores - only the two empty folders themselves are tracked,
so a fresh clone has somewhere to put charts. Tests build their own PDFs
rather than shipping a real one. Check `git status` before you commit.
